...
export function StackSection() {
  return (
    <section className="border-t border-[#111] bg-black py-[8vh]">
        ...
        <div className="mt-12 grid grid-cols-1 items-start gap-12 lg:grid-cols-[1fr_auto]">
          <div>
            {stackRows.map(([name, role]) => (
              <div key={name} className="flex items-center justify-between border-b border-[#111] py-4">
                <span className="font-mono text-[0.72rem] uppercase tracking-[0.14em] text-white">
                  {name}
                </span>
                <span className="font-mono text-[0.68rem] text-[#6d6d6d]">{role}</span>
              </div>
            ))}
          </div>

          <a
            href="https://github.com/Shizu0n/01-react-agent"
            target="_blank"
            rel="noreferrer"
            className="flex min-w-[260px] flex-col gap-5 rounded-2xl border border-white/[0.08] bg-[#040404] p-6 transition-colors duration-300 hover:border-[var(--accent-border)]"
          >
            <div className="flex items-center gap-3">
              <GithubIcon />
              <span className="font-mono text-[0.72rem] uppercase tracking-[0.16em] text-white">
                View Source
              </span>
            </div>
            <p className="text-[0.9rem] leading-[1.7] text-[#9d9d9d]">
              Full source, architecture docs, and deployment guide on GitHub.
            </p>
            <span className="self-start border border-[#262626] bg-transparent px-4 py-3 font-mono text-[0.72rem] uppercase tracking-[0.16em] text-white transition-all duration-300 hover:border-white hover:bg-white hover:text-black">
              View on GitHub →
            </span>
          </a>
        </div>
